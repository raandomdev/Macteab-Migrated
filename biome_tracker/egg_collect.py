...
# ------------------------------------------------------------------
# pyautogui helpers (macOS‑friendly, consistent)
# ------------------------------------------------------------------
def _pyautogui_click(x: int, y: int, clicks: int = 1, button: str = "left") -> None:
...
# ------------------------------------------------------------------
# Imports from fishing (must be the refactored macOS version)
# ------------------------------------------------------------------
from .fishing import (
...
def _replay_egg_path(
    *,
    ...
    for ev in events:
        ...
        delay = t * FORCE_SPEED / speed_multiplier
        target = start_time + delay
        now = time.time()
        if target > now:
            time.sleep(target - now)
        typ = ev.get("type", "")
        k = ev.get("key", "").lower().strip()
...
def _coerce_array_of_points(val: Any) -> list[tuple[int, int]]:
    if not isinstance(val, collections.abc.Iterable) or isinstance(val, (str, bytes)):
        return []
    res = []
    for item in val:
        pt = _coerce_point(item, [0, 0])
        if pt[0] > 0 or pt[1] > 0:
            res.append((pt[0], pt[1]))
    return res
...
def _run_single_egg_route(
    *,
    ...
    if not sleep_interruptible(1, 0.02):
        return False
    ...
    if is_first_route:
        ...
        if bool(cfg.get("fishing_mode", False)) and fish_close_x > 0 and fish_close_y > 0:
            try:
                _pyautogui_click(fish_close_x, fish_close_y)
            except Exception:
                pass
    ...
    if not sleep_interruptible(3.5, 0.02):
        return False
    ...
    if not sleep_interruptible(0.5, 0.02):
        return False
    ...
    try:
        ...
        time.sleep(0.1)
        pyautogui.moveTo(cam_x, cam_y + 75, duration=0.2)
        time.sleep(0.05)
    ...
    if not sleep_interruptible(0.75, 0.02):
        return False
    ...
    if not sleep_interruptible(2.25, 0.02):
        return False
    ...
    if not sleep_interruptible(1.25, 0.02):
        return False

    # Reset camera zoom
    try:
        _pyautogui_key_down("i")
