...
class RemoteMixin:
    ...
    def remote_use_item(self, item_name, amount):
        ...
        try:
            pyautogui.write(item_name.lower())
        except Exception:
            try:
                pyautogui.write(item_name.lower())
            except Exception:
    ...
    def remote_equip_aura(self, aura_name):
        ...
        if aura_search_bar and aura_search_bar[0] > 0:
            self.Global_MouseClick(aura_search_bar[0], aura_search_bar[1])
        time.sleep(step_delay)
        try:
            pyautogui.write(str(aura_name).lower())
        except Exception:
            try:
                pyautogui.write(str(aura_name).lower())
            except Exception:
        ...
        if first_aura_slot and first_aura_slot[0] > 0:
            try:
                ...
                try:
                    pyautogui.scroll(5000)  # positive = scroll up
                except Exception:
                    pass
                time.sleep(step_delay)
            except Exception:
