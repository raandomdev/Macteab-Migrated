...
class GuiMixin:
    def init_gui(self):
        ...
        try:
            if keyboard is not None and platform.system() != 'Darwin':
                keyboard.add_hotkey("F1", lambda: (_start_wrapper()))
                keyboard.add_hotkey("F2", lambda: (_stop_wrapper()))
            else:
                if platform.system() == 'Darwin':
                    print("[Hotkey] Skipping keyboard hotkeys on macOS (grant Accessibility to enable).")
        except Exception as _e:
            print(f"[Hotkey] Failed to bind F1/F2: {_e}")
    ...
    def create_potion_craft_tab(self, frame):
        ...
        try:
            ...
            print(f"[Hotkey] Failed to bind f3/f4: {_e}")
