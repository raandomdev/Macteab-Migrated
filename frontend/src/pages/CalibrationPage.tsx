...
const CALIBRATION_GROUPS: CalibrationGroup[] = [
    {
        ...
        fields: [
            ...
            { key: "exit_collections_drag", label: "Exit Collection Drag Position" },
    ...
    },
    {
        ...
        color: "#06b6d4",
        fields: [
            { key: "fishing_water_click", label: "Fishing Water Click Position" },
            { key: "fishing_spot_click", label: "Fishing Spot Click Position" },
            { key: "fishing_spot_second_click", label: "Fishing Spot Second Click Position" },
            { key: "walk_to_sell1", label: "Walk to Sell 1 Click Position" },
            { key: "walk_to_sell2", label: "Walk to Sell 2 Click Position" },
...
export default function CalibrationPage() {
    ...
    useEffect(() => {
        (window as any).onCalibrationResult = async (data: any) => {
            ...
            let latestConfig: any = config;
            try {
                if (window.pywebview?.api?.get_config) {
                    const fetched = await window.pywebview.api.get_config();
                    if (fetched && typeof fetched === "object") {
                        latestConfig = fetched;
                    }
                }
            } catch (err) {
            ...
            const mergedConfig = { ...(latestConfig || {}), [data.key]: value };
    ...
    useEffect(() => {
        fetch("https://raw.githubusercontent.com/raandomdev/Noteab-Macro/refs/heads/main/assets/macro_calibs_preset.json")
            .then(res => res.json())
            .then(data => {
    ...
    const availableModes = Array.from(new Set(presets.filter(p => p.resolution === selectedRes).map(p => p.mode)));

    const handleApplyClick = () => {
        const preset = presets.find(p => p.resolution === selectedRes && p.mode === selectedMode);
        if (!preset || !preset.calibrations) {
            alert("Please select a valid preset (Resolution and Mode)");
            return;
        }
    ...
    const exportCalibrations = async () => {
        if (!config) return;

        try {
            const calibrationData: Record<string, any> = {};
            CALIBRATION_GROUPS.forEach((group) => {
                group.fields.forEach((field) => {
                    if (Object.prototype.hasOwnProperty.call(config, field.key)) {
                        calibrationData[field.key] = config[field.key];
                    }
                });
            });

            const result = await window.pywebview?.api?.export_calibration_data_to_downloads?.(calibrationData);
            if (result?.success) {
                setExportStatus(`Saved to ${result.path}`);
                setTimeout(() => setExportStatus(""), 5000);
            } else {
                throw new Error(result?.error || "Unknown export error");
            }
        } catch (err) {
            console.error("Failed to export calibrations", err);
            setExportStatus("Failed to export calibrations");
            setTimeout(() => setExportStatus(""), 5000);
        }
    };
    ...
    return (
        <>
            ...
            <div className="info-banner" style={{ marginBottom: "16px" }}>
                <div>ℹ️ You can manually set up your calibrations by pressing "SELECT POS" or "SELECT REGION" within the pages.</div>
                <div style={{ marginTop: "6px" }}>
                    If you have troubles understanding refer to this tutorial (works for both WinOS and macOS):{" "}
            ...
            <div className="card" style={{ marginBottom: "16px" }}>
                <div className="card-header">
                    ...
                    <div style={{ flex: 1 }}>
                        ...
                        <p>Pre-made macro calibrations for common screen resolutions and window modes (so u don't have to set it up manually :umamusume_mambo_dancing:)</p>
                    </div>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr auto auto", gap: "10px", alignItems: "end", marginTop: "12px" }}>
                    <div>
                    ...
                    <button
                        ...
                        disabled={!selectedRes || !selectedMode}
                        style={{ height: "36px", whiteSpace: "nowrap" }}
                    >
                        Apply Preset
                    </button>
